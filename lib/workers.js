...
workers.log = (
  ...
  // Determine thr name of the log file
  const logFileName = originalCheckData.id;

  // Append the log string of the file name
  ...
  _logs.list(false, (err, logs) => {
    if (!err && logs && logs.length > 0) {
      logs.forEach(logName => {
        ...
        var newFiledId = logId + "-" + Date.now();
        _logs.compress(logId, newFiledId, err => {
          if (!err) {
            // Truncate the flag;
            _logs.truncate(logId, err => {
              if (!err) {
                console.log("Sucess truncating the logfile!!");
              } else {
...
workers.init = () => {
  ...
  // Call the compresssion loop so checks will execute later on
  workers.logRotationLoop();
};
// Export the module
